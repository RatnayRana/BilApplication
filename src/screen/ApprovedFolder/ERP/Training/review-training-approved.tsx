...
const ReviewTrainingApprovedScreen: React.FC<approvedScreen> = ({ route }) => {
    ...
    const {
        ...
        mutationFn: async (credentials: CreateTrainingAttributes) => {
            try {
               let data;
                if (credentials.training_status === 12) {
                    data = await apiClient.post(ERPURL.approvedTraining, credentials);
                } else {
                    data = await apiClient.post(ERPURL.trainingVerification, credentials);
                }
                if (data) {
                    setOpenDialog(true);
    ...
    const handleSubmit = (values: CreateTrainingAttributes) => {
        ...
        console.log(TrainingApprovalData, 'TrainingApprovalData');
...
export default ReviewTrainingApprovedScreen;
